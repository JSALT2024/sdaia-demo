...
def create_mediapipe_models(checkpoint_folder: str, min_confidence: float = 0.4) -> (object, object, object, object):
    ...
    with open(hand_model_path, 'rb') as file:
        hand_model_data = file.read()
    hand_options = vision.HandLandmarkerOptions(
        base_options=BaseOptions(model_asset_buffer=hand_model_data),
        min_hand_detection_confidence=min_confidence,
        min_hand_presence_confidence=min_confidence,
        num_hands=num_poses * 2)
    hand_detector = vision.HandLandmarker.create_from_options(hand_options)

    with open(pose_model_path, 'rb') as file:
